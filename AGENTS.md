...
0. Before writing any page's frontmatter, derive its three hierarchical tags: life-area and life-area/domain-name from the domain field, plus subject read from that domain's overview.md frontmatter. Place all three first in tags, in that order. If the domain is a coding project, also derive its programming-language tag(s) by scanning the actual source files under raw/ (not build/config file extensions) and append one tag per language — see SCHEMA.md. When choosing subject, don't let it duplicate a language tag the domain already has (see SCHEMA.md); if the domain's topic is genuinely two independent facets (e.g. platform + genre), add both as separate atomic tags instead of cramming them into subject. Before finishing any page, confirm it links back to [[wiki/domains/<life-area>/<domain>/overview]] using the full path.
...
To create a new domain: copy wiki/domains/_template/ into wiki/domains/<life-area>/<new-name>/, fill in overview.md including its subject field (the plain-language topic name, not the course code — e.g. subject: statistik even if the folder is hsm-03-statistik), add an entry to wiki/index.md under "Active domains."
