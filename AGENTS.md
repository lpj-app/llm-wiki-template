...
0. Before writing any page's frontmatter, derive its three hierarchical tags: life-area and life-area/domain-name from the domain field, plus subject read from that domain's overview.md frontmatter. Place all three first in tags, in that order. Before finishing any page, confirm it links back to [[wiki/domains/<life-area>/<domain>/overview]] using the full path.
